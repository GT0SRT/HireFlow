...
const buildJourneyStages = (app: Application) => {
  ...
  const assessmentPlan =
    app.assessment ||
    app.job?.job_description?.assessment_plan?.map((step) => ({
      testType: step.test_type,
      coveredTopics: step.focus_topics,
      suggestedDurationMinutes: step.suggested_duration_minutes,
    })) ||
    [];
  const interviewPlan = app.interviews?.length
    ? app.interviews
    : app.job?.job_description?.interview_plan || [];
  ...
  assessmentPlan.forEach((step, index) => {
    ...
    const completed = Boolean(
      savedStep?.score != null || savedStep?.completedAt,
    );
    const locked =
      screening?.status === "Not Shortlisted" ||
    ...
    stages.push({
      ...
      status: stageStatus,
      label:
        savedStep?.score != null
          ? `${savedStep.score}%`
          : step.suggestedDurationMinutes
            ? `${step.suggestedDurationMinutes} min`
            : stepStatusLabel[stageStatus],
    });
  });
            ...
            >
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2">
                    <Progress
                      value={app.progress ?? 10}
                      className="h-1.5 md:h-2"
                    />
                    <p className="text-xs text-muted-foreground mt-1">
                      {app.progress ?? 10}% complete
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 md:gap-3 shrink-0">
                  <Badge
                    variant="outline"
                    className={`hidden sm:inline-flex gap-1.5 ${st.color}`}
                  >
              ...
              {isOpen && (
                <div className="border-t border-border/50 p-4 md:p-6 space-y-6 animate-fade-in">
                  ...
                  <div>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
                      ...
                      {(() => {
                        ...
                        const assessments =
                          app.assessment || app.assessments || [];
                        const nextAssessmentIndex = assessments.findIndex(
                          (a: AssessmentStep | null) =>
                            !a?.score && !a?.completedAt,
                        );
                        if (nextAssessmentIndex >= 0) {
                          return (
                            <Button
                              size="sm"
                              className="w-full sm:w-auto glow-primary-sm gap-2"
                              onClick={() =>
                                navigate(
                                  `/candidate/applications/${app._id}/assessment/${nextAssessmentIndex}`,
                                )
                              }
                            >
                              Next Stage: Start Assessment{" "}
                        ...
                        const nextInterviewIndex = interviews.findIndex(
                          (i: InterviewStep) => {
                        ...
                        if (nextInterviewIndex >= 0) {
                          return (
                            <Button
                              ...
                              onClick={() =>
                                navigate(
                                  `/candidate/applications/${app._id}/interview/${nextInterviewIndex}`,
                                )
                              }
                            >
                              Next Stage: Join Interview{" "}
                              {nextInterviewIndex + 1}
                            </Button>
                          );
                        }
                        return null;
                    ...
                    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                      {journeyStages.map((stage) => (
                        <div
                          key={stage.key}
                          className={`rounded-xl border p-4 ${getStageTone(stage.status)}`}
                        >
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <Badge
                              variant="secondary"
                              className="text-[11px] shrink-0 max-w-xs truncate"
                            >
                              {stage.label}
                            </Badge>
                          </div>
                  ...
                  {/* Retry Resume */}
                  {(app.screening?.status === "Not Shortlisted" ||
                    app.status === "Rejected") && (
                    <div className="rounded-2xl border border-border/60 bg-background/60 p-5 md:p-6 space-y-4">
                      {!app.screening?.attempts ||
                      app.screening.attempts < 3 ? (
                        <div>
                          <h3 className="font-semibold text-sm mb-2">
                            Try Again with Another Resume
                          </h3>
                          <p className="text-xs text-muted-foreground mb-4">
                            You can upload a different resume and re-apply for
                            this position. (Attempt{" "}
                            {app.screening?.attempts || 1} of 3)
                          </p>
                                ...
                                try {
                                  ...
                                  if (data.screening?.parsedResume) {
                                    localStorage.setItem(
                                      cacheKey,
                                      JSON.stringify(
                                        data.screening.parsedResume,
                                      ),
                                    );
                                  }

                                  if (
                                    data.screening?.status === "Shortlisted"
                                  ) {
                                    toast.success(
                                      "Great! You've been shortlisted with the new resume! 🎉",
                                    );
                                  } else {
                                    toast.info(
                                      "The new resume didn't pass screening. You can try again.",
                                    );
                                  }

                                  const { data: updatedApps } =
                                  ...
                                  if (error instanceof Error) {
                                    ...
                                    errorMessage =
                                      resp?.data?.message || errorMessage;
                                  }
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            You have exhausted all 3 attempts to clear the
                            resume screening for this job.
                          </p>
                        </div>
                      )}
                    </div>
