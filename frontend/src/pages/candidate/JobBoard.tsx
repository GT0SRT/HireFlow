  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
        {jobs.map((job, i) => (
          <div key={job.id} className="glass rounded-2xl p-6 glass-hover animate-fade-in flex flex-col" style={{ animationDelay: `${i * 0.05}s` }}>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {job.skills.map(s => (
                <Badge key={s} variant="outline" className="text-xs bg-primary/5 border-primary/20 text-primary">{s}</Badge>
              ))}
            </div>
            ...
            <Button className="mt-5 w-full glow-primary-sm" onClick={() => toast.success(`Applied to ${job.title}!`)}>
              Apply Now
            </Button>
          </div>
        ))}
